"""Production environment/startup safety checks for the worker and beat
services - both run this at import time of worker.celery_app (`celery -A
worker.celery_app worker` / `... beat`), so a critical failure refuses to
start either process in production.

Distinct from worker.config_check (which only checks database/redis
connectivity): this module reads raw environment variable strings directly
via `os.environ` (or an injected mapping, for tests) and validates
configuration shape/safety without any I/O - ADMIN_TOKEN strength,
DATABASE_URL's default password, SCRAPING_MODE, the MARKET_WORKFLOW_* beat
schedule vars Celery Beat reads in worker.celery_app._build_beat_schedule,
and Telegram config.

services/api/app/core/env_validation.py is this module's mirror for the API
service - api and worker are separate deployable services with no shared
dependency in this repo, so it's a deliberate duplicate, not an import. Keep
the two in sync: same check names/shape, so GET /admin/env-check and this
worker's startup log are directly comparable.

Rules, in one place:

- Required in production: APP_ENV=production, DATABASE_URL, REDIS_URL,
  ADMIN_TOKEN (not the local-dev default, >= 32 chars). The worker itself
  never checks ADMIN_TOKEN (that's API-only auth) - it's validated here
  anyway because worker/beat share the same .env.production as the api
  container (see docker-compose.prod.yml), so a bad ADMIN_TOKEN there is
  still worth catching at worker/beat startup, not just the API's.
...
MARKET_WORKFLOW_SCHEDULE_CHECK_NAMES = frozenset(
    {
        "market_workflow_source_valid",
        "market_workflow_limit_valid",
        "market_workflow_hour_utc_valid",
        "market_workflow_minute_utc_valid",
        "market_workflow_enabled_boolean",
        "market_workflow_send_telegram_boolean",
        "market_workflow_telegram_ready",
    }
)


def validate_market_workflow_schedule(env: Mapping[str, str] | None = None) -> EnvValidationReport:
    """Beat-specific subset of validate_environment(): just the
    MARKET_WORKFLOW_* vars that worker.celery_app._build_beat_schedule reads
    to build the scheduled run-market-workflow entry. Beat never touches
    ADMIN_TOKEN/DATABASE_URL/SCRAPING_MODE itself, but those still get
    checked at import time via validate_environment() in worker.celery_app -
    this narrower report exists for beat-specific startup logging that talks
    about the schedule it's about to run, not the whole environment."""
    report = validate_environment(env)
    schedule_checks = [c for c in report.checks if c.name in MARKET_WORKFLOW_SCHEDULE_CHECK_NAMES]
    return EnvValidationReport(app_env=report.app_env, checks=schedule_checks)
