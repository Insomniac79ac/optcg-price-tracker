...
class TelegramSendError(Exception):
    """Raised when a Telegram alert could not be sent - missing config, a
    network error, or a non-2xx response. Callers (check_alerts) must catch
    this and mark the alert_events row as failed rather than let it crash
    the worker."""
