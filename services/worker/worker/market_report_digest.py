...
market intelligence report. Mirrors
services/api/app/services/telegram_market_digest.py exactly (same format,
same dedup semantics) - the worker has no shared code with the api service
...
Reads only fields the report already computed (worker/market_report.py) -
no new scoring/valuation formulas, no scraping, no AI/LLM-generated text.
"""
...
from worker.models import MarketIntelligenceReport, MarketReportDigestSend
...
def send_market_report_digest(
    ...
    been generated - callers should treat that as a successful no-op, not an
    error.
