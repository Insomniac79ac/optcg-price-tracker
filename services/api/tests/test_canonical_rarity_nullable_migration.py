"""Structural coverage for c7e91a4d2b60_make_canonical_rarity_nullable.
...
rows, is in test_canonical_rarity_nullable_migration_postgres.
"""
...
MIGRATION_PATH = VERSIONS_DIR / "c7e91a4d2b60_make_canonical_rarity_nullable.py"

REVISION = "c7e91a4d2b60"
DOWN_REVISION = "a9f31c7d5b64"
...
def _load_migration():
    spec = importlib.util.spec_from_file_location("canonical_rarity_migration", MIGRATION_PATH)
...
def _run(direction: str, *, null_rows: int = 0, total_rows: int = 15):
    ...
    def execute(clause, *args, **kwargs):
        ...
        result.all.return_value = [("EB03-003",), ("OP05-100",)]
        return result
...
def test_the_revision_sits_on_the_asset_variant_contract_head():
    module = _load_migration()
...
def test_upgrade_touches_nothing_else():
    ...
    # No data statement, and no synthetic value anywhere near it.
    for forbidden in ("UPDATE", "INSERT", "DELETE", "COALESCE", "UNKNOWN", "'N/A'"):
...
def test_downgrade_restores_not_null_when_the_data_allows_it():
    ...
    assert [c[0] for c in calls] == ["alter_column"]
    _, table, column, kwargs = calls[0]
    assert (table, column) == ("canonical_cards", "rarity")
    assert kwargs["nullable"] is False


def test_downgrade_aborts_before_any_ddl_when_a_rarity_is_null():
    ...
    with pytest.raises(RuntimeError) as excinfo:
        _run("downgrade", null_rows=18)

    message = str(excinfo.value)
    assert REVISION in message
    assert "18 canonical_cards row(s)" in message
    # It names the codes an operator has to decide about.
    assert "EB03-003" in message
...
def test_the_refusal_explains_where_the_authoritative_rarity_lives():
    """A refusal that does not say what to do instead gets worked around."""
    with pytest.raises(RuntimeError) as excinfo:
        _run("downgrade", null_rows=1)

    assert "official_rarity" in str(excinfo.value)
