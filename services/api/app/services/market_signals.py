...
def _owned_signals(
    ...
    """At most one owned_above_target_sell and one owned_below_cost_basis
    signal per card - the response has no per-collection-item identifier, so
    multiple qualifying items for the same card collapse into a single
    card-level signal (using the first qualifying item's reference price)."""
    signals: list[_CandidateSignal] = []
    ...
    if above_target_item is not None:
        gap_jpy = value_jpy - above_target_item.target_sell_price_jpy
        gap_pct = _pct(gap_jpy, above_target_item.target_sell_price_jpy)
        signals.append(
            _CandidateSignal(
                ...
                **latest_trio,
                ...
                gap_pct=gap_pct,
                message=(
                    f"Owned {_card_label(card)} reached its target sell price: "
                    f"{value_jpy} JPY via {basis_label} vs target "
                    f"{above_target_item.target_sell_price_jpy} JPY."
                ),
                suggested_action="review_sell_opportunity",
            )
        )

    below_cost_item = next(
        (
            item
            for item in card_items
    ...
    if below_cost_item is not None:
        gap_jpy = value_jpy - below_cost_item.purchase_price_jpy
        gap_pct = _pct(gap_jpy, below_cost_item.purchase_price_jpy)
        signals.append(
            _CandidateSignal(
                ...
                **latest_trio,
                ...
                gap_pct=gap_pct,
                message=(
                    f"Owned {_card_label(card)} is below cost basis: "
                    f"{value_jpy} JPY via {basis_label} vs purchase "
                    f"{below_cost_item.purchase_price_jpy} JPY ({gap_pct:.2f}%)."
                ),
                suggested_action="monitor_drop",
            )
        )
        ...
        if not source_observations:
            signals.append(
                _CandidateSignal(
                    ...
                    **latest_trio,
                    message=(
                        f"{label} has an active {source_name} mapping but no price "
                        "observations yet."
                    ),
        ...
        if age > stale_after:
            signals.append(
                _CandidateSignal(
                    ...
                    **latest_trio,
                    message=(
                        f"{label} {source_name} price is stale - last observed "
                        f"{latest_obs.observed_at.isoformat()}."
                    ),
