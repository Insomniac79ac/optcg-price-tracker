...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/analytics/digest/reports/${id}`;
  ...
  try {
    ...
    console.error(
      `[analytics-digest-report-detail proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[analytics-digest-report-detail proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
