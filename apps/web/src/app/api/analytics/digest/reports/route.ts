...
export async function GET(request: NextRequest) {
  const backendUrl = `${API_INTERNAL_URL}/analytics/digest/reports${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[analytics-digest-reports proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[analytics-digest-reports proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
