...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/analytics/portfolio-risk${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[portfolio-risk proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[portfolio-risk proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
