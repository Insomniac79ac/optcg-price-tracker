...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/analytics/wishlist${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[wishlist-analytics proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[wishlist-analytics proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
