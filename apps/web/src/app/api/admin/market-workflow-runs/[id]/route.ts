...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/market-workflow-runs/${id}`;
  ...
  try {
    ...
    console.error(
      `[market-workflow-run-detail proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[market-workflow-run-detail proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
