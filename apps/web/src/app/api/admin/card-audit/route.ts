...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/card-audit`;
  console.log(`[card-audit proxy] API_INTERNAL_URL=${API_INTERNAL_URL}`);
  console.log(`[card-audit proxy] fetching backend URL=${backendUrl}`);
  ...
  try {
    ...
    console.error(
      `[card-audit proxy] failed to reach backend at ${backendUrl}: ${
  ...
  console.log(`[card-audit proxy] backend status=${backendResponse.status}`);
  console.log(
    `[card-audit proxy] response body length=${bodyText.length}`,
  );

  // Whatever the backend sent, an empty or non-JSON body means this route
  // can't hand the frontend the report it expects - surface that explicitly
  // instead of forwarding a body the page can't parse.
  ...
  try {
    ...
    console.error(
      `[card-audit proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
