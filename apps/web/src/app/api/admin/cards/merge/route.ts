...
// A real (dry_run=false) merge reassigns foreign keys across several
// tables - longer timeout than a simple mutation.
...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards/merge`;
  ...
  try {
    ...
    console.error(
      `[cards-merge proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[cards-merge proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
