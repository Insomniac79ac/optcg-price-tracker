...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards/${id}/merge-preview${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[cards-merge-preview proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[cards-merge-preview proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
