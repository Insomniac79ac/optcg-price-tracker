...
export async function POST(request: NextRequest) {
  ...
  // is forwarded byte-for-byte without us re-encoding it. See
  // src/app/api/admin/backup/restore/route.ts for the same pattern.
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards/import.csv${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[admin-cards-import proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[admin-cards-import proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
