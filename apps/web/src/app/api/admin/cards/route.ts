...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[admin-cards proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[admin-cards proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
