...
// Builds a full merge preview (affected records + field diff) for each of
// the top duplicate pairs - longer timeout than a simple list endpoint.
...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards/duplicates/bulk-preview`;
  ...
  try {
    ...
    console.error(
      `[cards-duplicates-bulk-preview proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[cards-duplicates-bulk-preview proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
