...
// Duplicate detection scores every candidate pair in the (filtered) catalog -
// longer timeout than a simple list endpoint.
...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/cards/duplicates${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[cards-duplicates proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[cards-duplicates proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
