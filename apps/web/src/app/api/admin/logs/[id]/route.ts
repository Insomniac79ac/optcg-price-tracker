...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/logs/${id}`;
  ...
  try {
    ...
    console.error(
      `[admin-log-detail proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[admin-log-detail proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
