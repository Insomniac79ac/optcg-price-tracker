...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/snkrdunk-candidates/${id}/rematch`;
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-rematch proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-rematch proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
