...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/snkrdunk-candidates/${id}/matches`;
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-matches proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-matches proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
