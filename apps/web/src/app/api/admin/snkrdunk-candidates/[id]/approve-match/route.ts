...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/snkrdunk-candidates/${id}/approve-match`;
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-approve-match proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-approve-match proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
