...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/snkrdunk-candidates/${id}/reject-match`;
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-reject-match proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-reject-match proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
