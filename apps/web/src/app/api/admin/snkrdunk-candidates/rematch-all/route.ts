...
// Longer than the per-candidate routes - this can score every unresolved
// candidate against every canonical card in one request.
...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/snkrdunk-candidates/rematch-all`;
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-rematch-all proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[snkrdunk-candidates-rematch-all proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
