...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/import-validation/reports/${id}`;
  ...
  try {
    ...
    console.error(
      `[import-validation-report-detail proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[import-validation-report-detail proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
