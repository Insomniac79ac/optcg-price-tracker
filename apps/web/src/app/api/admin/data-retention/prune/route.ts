...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/data-retention/prune`;
  ...
  try {
    ...
    console.error(
      `[data-retention-prune proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[data-retention-prune proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
