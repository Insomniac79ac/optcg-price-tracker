...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/job-locks/cleanup-expired`;
  ...
  try {
    ...
    console.error(
      `[job-locks-cleanup-expired proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[job-locks-cleanup-expired proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
