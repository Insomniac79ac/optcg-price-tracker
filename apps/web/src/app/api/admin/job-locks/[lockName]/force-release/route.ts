...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/job-locks/${lockName}/force-release`;
  ...
  try {
    ...
    console.error(
      `[job-locks-force-release proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[job-locks-force-release proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
