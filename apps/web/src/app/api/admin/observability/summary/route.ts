...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/observability/summary`;
  ...
  try {
    ...
    console.error(
      `[observability-summary proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[observability-summary proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
