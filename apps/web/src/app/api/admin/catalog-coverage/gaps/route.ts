...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/catalog-coverage/gaps${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[catalog-coverage-gaps proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[catalog-coverage-gaps proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
