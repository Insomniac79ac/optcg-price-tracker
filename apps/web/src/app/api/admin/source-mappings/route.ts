...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/source-mappings${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[source-mappings proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[source-mappings proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
