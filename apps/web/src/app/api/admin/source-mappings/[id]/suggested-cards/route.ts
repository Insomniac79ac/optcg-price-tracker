...
export async function GET(
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/source-mappings/${id}/suggested-cards`;
  ...
  try {
    ...
    console.error(
      `[source-mappings-suggested-cards proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[source-mappings-suggested-cards proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
