...
// Longer than the other routes - this can re-score every selected mapping
// against the full card catalog in one request.
...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/admin/source-mappings/recheck-quality`;
  ...
  try {
    ...
    console.error(
      `[source-mappings-recheck-quality proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[source-mappings-recheck-quality proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
