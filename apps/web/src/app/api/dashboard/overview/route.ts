...
export async function GET() {
  ...
  const backendUrl = `${API_INTERNAL_URL}/dashboard/overview`;
  ...
  try {
    ...
    console.error(
      `[dashboard-overview proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[dashboard-overview proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
