...
// CSV imports can take longer than a typical request (parsing + validating
// every row) - a generous 60s timeout vs. the 15s used elsewhere.
...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/wishlist/import.csv${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[wishlist-import proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[wishlist-import proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
