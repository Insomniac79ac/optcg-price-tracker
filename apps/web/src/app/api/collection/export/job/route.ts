...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/collection/export.csv/job`;
  ...
  try {
    ...
    console.error(
      `[collection-export-job proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[collection-export-job proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
