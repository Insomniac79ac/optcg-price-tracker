...
export async function POST(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/collection/import.csv${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[collection-import proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[collection-import proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
