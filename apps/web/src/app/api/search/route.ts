...
export async function GET(request: NextRequest) {
  ...
  const backendUrl = `${API_INTERNAL_URL}/search${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[search proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[search proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
