...
export async function GET(request: NextRequest) {
  const backendUrl = `${API_INTERNAL_URL}/market/signals${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[market-signals proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[market-signals proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
