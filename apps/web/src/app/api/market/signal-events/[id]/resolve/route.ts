...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/market/signal-events/${id}/resolve`;
  ...
  try {
    ...
    console.error(
      `[signal-event-resolve proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[signal-event-resolve proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
