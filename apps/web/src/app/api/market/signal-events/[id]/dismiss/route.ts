...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/market/signal-events/${id}/dismiss`;
  ...
  try {
    ...
    console.error(
      `[signal-event-dismiss proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[signal-event-dismiss proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
