...
export async function POST(
  ...
  const backendUrl = `${API_INTERNAL_URL}/market/signal-events/${id}/watch`;
  ...
  try {
    ...
    console.error(
      `[signal-event-watch proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[signal-event-watch proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
