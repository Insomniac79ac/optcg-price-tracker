...
export async function PATCH(
  ...
  const backendUrl = `${API_INTERNAL_URL}/market/signal-events/${id}`;
  ...
  try {
    ...
    console.error(
      `[signal-event-patch proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[signal-event-patch proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
