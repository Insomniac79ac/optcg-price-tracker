...
export async function GET(request: NextRequest) {
  const backendUrl = `${API_INTERNAL_URL}/market/opportunities${request.nextUrl.search}`;
  ...
  try {
    ...
    console.error(
      `[market-opportunities proxy] failed to reach backend at ${backendUrl}: ${
  ...
  try {
    ...
    console.error(
      `[market-opportunities proxy] backend response was not valid JSON (status=${backendResponse.status})`,
    );
    return NextResponse.json(
      {
