...
import { FULL_MATCHER } from "@/lib/proxyGuard";

// proxy.ts can't import FULL_MATCHER into its `config.matcher` field -
...
describe("proxy.ts config.matcher", () => {
  it("stays identical to proxyGuard.FULL_MATCHER", () => {
    ...
    expect(entries).toEqual(FULL_MATCHER);
  });
});
